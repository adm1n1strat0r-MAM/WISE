...
document.addEventListener("DOMContentLoaded", () => {
  firebase.auth().onAuthStateChanged((user) => {
    if (!user) {
      // User is not logged in, show a customized alert using SweetAlert2
      Swal.fire({
        icon: 'warning',
        title: 'Not Logged In',
        ...
        if (result.isConfirmed) {
          // Redirect the user to the login page or take any other desired action
              ...
              if (
                ...
                if (months > 2) {
                  Swal.fire({
                    icon: 'info',
                    title: 'Quiz Attempt',
                    text: 'You can attempt the quiz again.',
                    confirmButtonColor: '#3085d6',
                    confirmButtonText: 'OK'
                  });
                } else {
                  Swal.fire({
                    icon: 'error',
                    title: 'Quiz Attempt',
                    text: 'You cannot attempt the quiz again.',
                    confirmButtonColor: '#3085d6',
                    confirmButtonText: 'OK'
                  }).then(() => {
      ...
      function updateTimer() {
        if (seconds > 0) {
          ...
          const formattedTime = `${pad(hours)}:${pad(minutes)}:${pad(seconds % 60)}`;
      ...
      function showTimesUpAlert() {
        Swal.fire({
          icon: 'warning',
          title: "Time's Up!",
          text: 'Sorry, the time limit for the quiz has been reached.',
          confirmButtonColor: '#3085d6',
          confirmButtonText: 'OK'
        }).then(() => {
          ...
          .then(() => {
            if (percentage <= 50) {
              Swal.fire({
                icon: 'warning',
                title: 'Quiz Result',
                text: 'Your test score was below 50%. You can try again after 2 months',
                confirmButtonColor: '#3085d6',
                confirmButtonText: 'OK'
              }).then(() => {
                window.location = "http://localhost:4000/index";
